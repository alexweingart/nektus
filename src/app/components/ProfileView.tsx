...
const ProfileView: React.FC = () => {
    ...
    if (profile) {
      setLocalProfile(prev => {
        ...
        console.log('ProfileView: Updating local profile', {
          ...
          currentStateBio: bio || '[empty]',
          hasProfileBio: !!profile.bio,
          hasPrevBio: !!prev.bio,
          willPreserveBio: !profile.bio && (!!prev.bio || !!bio)
        });
        ...
        if (bioToUse && bioToUse !== bio) {
          console.log('Updating bio state from profile update logic:', bioToUse);
          setBio(bioToUse);
      ...
      try {
        ...
        if (currentProfile) {
          ...
          if (currentProfile.bio) {
            console.log('Setting bio from profile:', currentProfile.bio);
            setBio(currentProfile.bio);
            hasLoadedBio.current = true;
          } else {
            // The bio will be generated by ProfileContext if needed
            console.log('No bio in profile, using placeholder');
  ...
  useEffect(() => {
    ...
    if (profile?.bio && (bio === PLACEHOLDER_BIO || !hasLoadedBio.current)) {
      console.log('Updating bio from profile change:', profile.bio);
      setBio(profile.bio);
      hasLoadedBio.current = true;
    } else if (profile?.bio) {
      // Log that we're not updating the bio even though profile.bio exists
      console.log('Not updating bio from profile change because bio already exists:', {
        profileBio: profile.bio,
        currentBio: bio,
        hasLoadedBio: hasLoadedBio.current
      });
    }
  }, [profile?.bio, bio]);
