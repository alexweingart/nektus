...
export async function POST(request: NextRequest) {
  try {
    ...
    const requestingUserTimezone = await adminGetUserTimezoneById(decodedToken.uid);
    console.log(`🌍 Using timezone: ${requestingUserTimezone || 'UTC (fallback)'} for user ${decodedToken.uid}`);
        ...
        (async () => {
          try {
            ...
            switch (validTokens.provider) {
              case 'google':
                const googleToken = await validateAndRefreshGoogleToken(validTokens);
                if (!googleToken) {
...
async function validateAndRefreshGoogleToken(tokens: CalendarTokens): Promise<string | null> {
  try {
    const accessToken = tokens.accessToken;
    const expiresAt = tokens.expiresAt;
    ...
    if (!accessToken || typeof accessToken !== 'string') {
      ...
      // Update tokens in database (we'll need to create this function)
      // await adminUpdateGoogleTokens(userEmail, refreshedData.accessToken, refreshedData.expiresAt);
