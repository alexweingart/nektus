...
export const editEventFunction: OpenAIFunction = {
  name: 'editEvent',
  description: 'Edit an existing event that was just created. Use this when the user wants to modify a previous suggestion (change time, date, place, etc.). Supports both definitive changes ("change to 3pm") and exploratory/conditional changes ("do I have earlier times?").',
  parameters: {
    type: 'object',
    properties: {
      changeType: {
      ...
      timePreference: {
        ...
        description: 'For time changes: "earlier" for "do I have earlier times?", "later" for "can we do it later?", "specific" for exact times. Only provide when changeType is "time".',
      },
      newPreferredSchedulableDates: {
        type: 'object',
        description: `New date range if user wants to change the day. Only provide if changeType includes "date". Today is ${new Date().toISOString().split('T')[0]}. IMPORTANT: When user says "next week" or "the week after", calculate dates RELATIVE TO THE CURRENTLY SCHEDULED EVENT (from conversation history), NOT relative to today. If event is on Oct 11 and user says "next week", that means Oct 18. If they then say "the week after", that means Oct 25.`,
        properties: {
          startDate: {
            type: 'string',
            format: 'date',
      ...
      newPreferredSchedulableHours: {
        type: 'object',
        description: 'New time windows if user wants a different time of day. Only provide if changeType includes "time". IMPORTANT: For explicit times like "12:00 PM", set BOTH start AND end to the SAME time (e.g., {start: "12:00", end: "12:00"}) to indicate exact time request.',
        properties: {
      ...
      newPlaceIndex: {
        type: 'number',
        description: 'Index of different place from cached options if user wants to change to one of the suggested alternatives. Only provide if changeType includes "place" AND user is selecting from existing suggestions.',
      },
      newPlaceType: {
        type: 'string',
        description: 'Type of new venue if user wants a completely different kind of place (e.g., "park", "cafe", "library"). Only provide if changeType includes "place" AND user requests a different type of venue than what was originally suggested.',
      },
      newDuration: {
        type: 'number',
        description: 'New duration in minutes if user wants to change how long the event is. Only provide if changeType includes "duration".',
      },
    },
    required: ['changeType'],
  },
...
export function processEditEventResult(args: string): EditEventResult {
  try {
    const parsed = JSON.parse(args);

    const result: EditEventResult = {
      changeType: parsed.changeType,
    };

    if (parsed.isConditional !== undefined) {
      result.isConditional = parsed.isConditional;
    }
    ...
    console.error('Error parsing editEvent result:', _error);
    throw new Error('Failed to parse edit event');
  }
}