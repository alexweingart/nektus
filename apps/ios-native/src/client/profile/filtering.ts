/**
 * Profile filtering utilities for iOS
 * Adapted from: apps/web/src/client/profile/filtering.ts
 *
 * Changes from web:
 * - Updated imports for shared-types
 * - No other changes needed (pure functions)
 */
...
function filterContactEntriesByCategory(
  ...
  return contactEntries.filter(
    (entry) => allowedSections.includes(entry.section) && entry.isVisible
  );
}
