...
import { EVENT_ENHANCEMENT_SYSTEM_PROMPT, buildEventEnhancementContext } from '@/lib/server/ai-scheduling/system-prompts';
...
import type { EventSearchResult } from '@/lib/server/ai-scheduling/helpers/search-events';
...
export async function handleSearchEvents(
  ...
  try {
    ...
    const { createWebSearchResponse } = await import('@/lib/server/ai-scheduling/openai-client');
