...
export const usePWAInstall = () => {
  ...
  useEffect(() => {
    const checkInstallation = async () => {
      ...
      // Show button for all mobile platforms when not installed
      // iOS will show manual instructions, Android will use native prompt or show fallback instructions
    ...
    const handleBeforeInstallPrompt = (e: Event) => {
      ...
      // NOW show the button since we have the prompt ready
      setIsInstallable(true);
      console.log('PWA: beforeinstallprompt event captured, install button enabled');
    };
