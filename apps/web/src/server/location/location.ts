...
export function generateGoogleMapsUrl(
  ...
  if (googlePlaceId && placeName) {
    // Best option: Use Place ID with coordinates for guaranteed accurate place card
    // Including coordinates prevents Google from treating this as a web search
    const encodedName = encodeURIComponent(placeName);
    const query = `${encodedName}+${coordinates.lat},${coordinates.lng}`;
    return `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(query)}&query_place_id=${googlePlaceId}`;
  } else if (placeName) {
